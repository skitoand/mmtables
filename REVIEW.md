...
**Бэкап:** `mmtable-PROD-BACKUP-20260913-165536-before-deploy.tar.gz`  
**Деплой:** GitHub Actions [Deploy Production #34770031410](https://github.com/skitoand/mmtables/actions/runs/34770031410)  
**Статус:** OK

### Что сделано
- Для выбранного объекта рисования включается «Линия» и показываются его реальные цвет и толщина.
- Изменения форматирования применяются к объекту и сохраняются.
- Исправлено восстановление штриха после обновления страницы или открытия документа.

### Проверка
- [x] `node --check app.js` и `node --check draw-tools.js` прошли.
- [x] Deploy Production #34770031410 success.
- [x] `https://mmtable.crystalsystems.ru/` вернул `200 OK`.

## 2026-09-13 — настройки цвета и толщины рисования

**Commit:** `b241601` — Связать рисование с настройками линии.  
**Бэкап:** `mmtable-PROD-BACKUP-20260913-164533-before-deploy.tar.gz`  
**Деплой:** GitHub Actions [Deploy Production #34769536715](https://github.com/skitoand/mmtables/actions/runs/34769536715)  
**Статус:** OK

### Что сделано
- Новые штрихи используют цвет и толщину из настроек «Линия» в панели форматирования.
- Цвет и толщина уже созданных объектов рисования изменяются через ту же панель.
- Обновлён cache-buster `draw-tools.js`.

### Проверка
- [x] `node --check app.js` и `node --check draw-tools.js` прошли.
- [x] Deploy Production #34769536715 success.
- [x] `https://mmtable.crystalsystems.ru/` вернул `200 OK`.
...
**Деплой:** GitHub Actions [Deploy Production #29913052467](https://github.com/skitoand/mmtables/actions/runs/29913052467)  
**Статус:** pending/OK

### Что сделано
- Клиент не сохраняет документ, если в памяти 0 листов
- `/api/layout` отклоняет `sheets: []` и near-empty overwrite большого документа
- Восстановлены **копии** (оригиналы не перезаписывались):
  - `ДЕИ 1С:УНФ (копия 22.07.2026 08:12)` — полная (~499 КБ, 4 листа)
  - `ДЕИ 1С:УНФ (копия 21.07.2026)` — более ранняя
  - также копия затёртого `Электрик 1 (бэкап 18.07.2026)`

### Причина потери
Автосохранение могло записать payload `{"schemaVersion":3,"activeSheetId":1,"sheets":[]}` поверх живого документа (гонка при открытии/смене файла, пока `documentSheetsState` ещё пустой).
...
**Бэкап:** `mmtable-PROD-BACKUP-20260722-083727-before-deploy.tar.gz`  
**Деплой:** GitHub Actions [Deploy Production #29904711504](https://github.com/skitoand/mmtables/actions/runs/29904711504)  
**Статус:** OK

### Что сделано
- Перед сменой документа: сброс отложенного autosave + сохранение текущего файла
- Persist пишет только в `documentId`, зафиксированный в момент сборки layout
- API `/api/layout` больше не падает обратно на session `active_document_id` без явного id
- Восстановлены документы:
  - `Электрик 1 (бэкап 18.07.2026)` ← снимок с утра 21.07
  - `Электрик 1` ← последняя чистая версия 18.07
  - копия `Электрик 1 (восстановлен 22.07.2026)`
- Повреждённые снимки сохранены отдельно для разбора

### Проверка
- [x] Deploy Production #29904711504 success
- [x] Восстановленные документы без маркеров «1С Розница»
...
## 2026-09-14 — надёжная синхронизация со Сферой

**Коммиты:** `dd2d36b04bf43e38db52a2d73aac3c00528a1bdc`, `8fb9c9e913b904a545c7e8f0f96f34173bad25f3`  
**GitHub Actions:** https://github.com/skitoand/mmtables/actions/runs/34833015423  
**Бэкап:** `/opt/apps/backups/mmtable-PROD-BACKUP-20260914-102435-before-deploy.tar.gz`  
**Статус:** OK

- Добавлены приёмник и обратная durable outbox-очередь синхронизации со Сферой: HMAC, журнал, идемпотентность, защита от конфликтов и устаревших событий, повторная доставка.
- В deployment-скрипт добавлена доставка нового модуля `sfera_sync.py`.
- Первичная выкладка `#34832697798` остановилась, потому что модуль не был включён в скрипт. Исправление опубликовано отдельным commit и успешно выложено workflow `#34833015423`.
